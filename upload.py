...
async def list_serial_ports():
    ...
    if system_platform == "Windows":
        ...
        available_ports = [port for port in os.listdir('/dev/') if 'cu.usbmodem' in port]
    ...
    for port in ports:
        if port in session_port_map:
            ...
            if window.current_tab:
                ...
                await new_session.async_set_variable("user.monitor_port",
                                                     port)  # Set the custom variable to identify the session
...
async def main(connection):
    ...
    parser = argparse.ArgumentParser(description="Flash and optionally monitor ESP devices.")
    ...
    parser.add_argument('--split', choices=['vertical', 'horizontal'], default='vertical',
                        help="Split direction for iTerm2 panes.")
